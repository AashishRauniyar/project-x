...
import OrderNowFab from "@/components/ui/OrderNowFab";
...
// Helper to extract the first image src from post content
...
// Helper to extract the first link (Order Now/Buy Now) from post content
...
export default async function PostPage({ params }: PostPageProps) {
  try {
    ...
    // For the sidebar rating, parse from table if not present in meta
    function extractOverallScoreFromContent(content: string): number | null {
      // Look for a table row with 'Overall Score' and a number (e.g., 4.7)
      const match = content.match(/Overall Score<\/[^>]+>\s*<[^>]+>\s*<[^>]+>([0-9]\.[0-9])/i);
      if (match && match[1]) {
        return parseFloat(match[1]);
      }
      // Try a more generic match for a number in a table cell with 'Overall Score'
      const genericMatch = content.match(/Overall Score[\s\S]*?(\d\.[0-9])/i);
      if (genericMatch && genericMatch[1]) {
        return parseFloat(genericMatch[1]);
      }
      return null;
    }
    // Map product data for ProductCard
    let rating = post.meta.rating || 0;
    if (!rating || rating === 0) {
      rating = extractOverallScoreFromContent(post.content.rendered) || 0;
    }
    // Use first image from content if available, else fallback
    const sidebarImage = extractFirstImageSrc(post.content.rendered) || post.meta.product_image || post._embedded?.["wp:featuredmedia"]?.[0]?.source_url || "/public/logo.svg";
    // Use first link from content as order url if available, else fallback
    const sidebarOrderUrl = extractFirstLink(post.content.rendered) || post.meta.order_url || "#";
    const productData = {
      id: post.id.toString(),
      title: post.title.rendered,
      image: sidebarImage,
      price: post.meta.price || "",
      shopUrl: sidebarOrderUrl,
      reviewUrl: post.link,
      description: post.meta.description || post.excerpt?.rendered?.replace(/<[^>]*>/g, "") || "",
      keyIngredients: post.meta.key_ingredients || [],
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 relative overflow-hidden">
        ...
        <main className="max-w-7xl mx-auto px-4 py-8 relative z-10">
          ...
          {/* Use a container to center the main content, and position the sidebar in the extra space on the right */}
          <div className="relative w-full flex justify-center">
            {/* Main content column, always centered */}
            <div className="w-full max-w-3xl mx-auto">
              {/* Sidebar for mobile */}
              <div className="block lg:hidden mb-8">
                <ProductCard product={productData} className="shadow-2xl border-2 border-primary-100 w-full" />
              </div>
              <article className="space-y-8">
                ...
                <header className="bg-white/80 backdrop-blur-xl rounded-3xl p-10 shadow-2xl border border-white/20 relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    ...
                    {/* In the main content, above the article title, show actual categories as badges */}
                    <div className="flex flex-wrap gap-2 mb-4">
                      {Array.isArray(post._embedded?.["wp:term"]?.[0]) && post._embedded["wp:term"][0].map((cat) => (
                        <a
                          key={cat.id}
                          href={`/category/${cat.slug}`}
                          className="inline-block px-4 py-1 rounded-full bg-blue-100 text-blue-700 font-semibold text-sm hover:bg-blue-200 transition-colors border border-blue-200"
                        >
                          {cat.name}
                        </a>
                      ))}
            ...
            {/* Fixed Sidebar for desktop, visually aligned */}
            <aside className="hidden lg:block">
              <div className="fixed top-28 right-12 w-[340px] max-w-xs z-40">
                <ProductCard product={productData} className="shadow-2xl border-2 border-primary-100 h-fit w-full" />
              </div>
            </aside>
          </div>
        </main>
        {/* Floating Action Button for mobile (client component) */}
        <OrderNowFab shopUrl={productData.shopUrl} />
      </div>
    );
  } catch (error) {
